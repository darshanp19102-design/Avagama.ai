...
async def dashboard(
    ...
    # Fetch counts from the persistent tracking sets
    domain_names_doc = await collection('use_case_tracking').find_one({'user_id': current_user['id'], 'type': 'domain'})
    total_domain_use_cases = len(domain_names_doc.get('names') or []) if domain_names_doc else 0

    company_names_doc = await collection('use_case_tracking').find_one({'user_id': current_user['id'], 'type': 'company'})
    total_company_use_cases = len(company_names_doc.get('names') or []) if company_names_doc else 0
    ...
    return {
        ...
        'total_domain_use_cases': total_domain_use_cases,
        'total_company_use_cases': total_company_use_cases,
