...
export default function EvaluatePage() {
  ...
  useEffect(() => {
    api.get('/api/auth/me').then(r => setUserLimits(r.data));
  }, []);
  ...
  const submit = async () => {
    ...
    try {
      ...
      setLoading(false);
      alert(err.response?.data?.detail || 'An error occurred during evaluation');
    }
  };

  if (loading) return <div><TopNav /><div className="loader">Evaluating your process...</div></div>;

  const count = userLimits?.evaluation_count ?? 0;
  const limit = userLimits?.evaluation_limit ?? 20;

  return <div><TopNav /><main className="page">
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <h1>Evaluate a process</h1>
      <div style={{ background: '#f0f0f0', padding: '8px 16px', borderRadius: '20px', fontWeight: 'bold', fontSize: '14px', color: count >= limit ? '#d33' : '#333' }}>
        Evaluations used: {count} / {limit}
      </div>
    </div>
